...
function testTextCore() {
  ...
  assert(prompt.prompt.includes('Translate these 1 game strings into German.'));
  ...
  const jsonParsed = parseBatchResponse('["Jaeger [[0]]"]', {
    expectedCount: 1,
    shieldMaps: [protectedValue.placeholders]
  });
  assert.strictEqual(jsonParsed[0], 'Jaeger %r%');
  ...
  assert.strictEqual(replacements[0].type, 'NAME_STRING');
