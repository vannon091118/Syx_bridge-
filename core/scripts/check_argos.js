...
// Failure cooldown with exponential backoff: after ETIMEDOUT, retry interval
// doubles (60s→120s→240s, cap 5min) to prevent 23× GUI-poll-spam.
let _lastArgosFailureTime = 0;
let _argosCooldownMs = 30000; // Start: 30s
const ARGOS_COOLDOWN_CAP_MS = 300000; // Cap: 5min

/**
 * Checks if Argos is in failure cooldown. If cooldown has expired,
 * resets the state so the next call can try again.
 * @returns {boolean}
 */
function isArgosOnCooldown() {
  if (_lastArgosFailureTime === 0) return false;
  const elapsed = Date.now() - _lastArgosFailureTime;
  if (elapsed < _argosCooldownMs) {
    return true;
  }
  // Cooldown expired — reset and try again
  _lastArgosFailureTime = 0;
  _argosCooldownMs = 30000; // Reset to base on success
  return false;
}

/**
 * Marks an Argos failure and starts/extends cooldown.
 * Also invalidates the installed cache so callers don't keep trying.
 * Cooldown doubles each failure: 30s → 60s → 120s → 240s → 300s (cap).
 */
function markArgosFailure() {
  _lastArgosFailureTime = Date.now();
  _argosCooldownMs = Math.min(_argosCooldownMs * 2, ARGOS_COOLDOWN_CAP_MS);
  argosInstalledCache = null;
  _pythonCache = null;
  console.warn(`[ARGOS] In cooldown für ${Math.round(_argosCooldownMs / 1000)}s — nächster Versuch in ${Math.round(_argosCooldownMs / 1000)}s`);
}
...
async function getAvailableArgosLanguages() {
  // Skip if in failure cooldown (prevents 23× ETIMEDOUT from GUI polling)
  if (isArgosOnCooldown()) {
    return [];
  }
  const python = getPython();
  ...
  try {
    ...
    // ETIMEDOUT → cooldown einleiten (verhindert 23× Wiederholungen)
    markArgosFailure();
    return [];
  }
}
...
async function checkArgosLanguages(targetLangs) {
  ...
  // Skip if in failure cooldown (verhindert GUI-Poll-Spam)
  if (isArgosOnCooldown()) {
