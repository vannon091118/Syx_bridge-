...
/**
 * Records inquirer.prompt calls. Each test gets its own fresh mock so we
 * can count exactly how often the gate asked the user.
 */
function makeInquirerMock(answer) {
  const calls = [];
  return {
    calls,
    instance: {
 ...
 */
function makeDeps({ inquirerInstance, get, set, configOverrides = {} }) {
  let confirmed = false;
  return {
    config: {
      NATIVE_MODE: true,
...
async function main() {
  ...
  try {
    ...
    setStdinIsTTY(true);
    const inq1 = makeInquirerMock(true);
    const capt1 = captureConsole();
    const ops1 = createRuntimeOps(makeDeps({ inquirerInstance: inq1.instance }));
    let t1Result = null;
    try {
    ...
    capt1.restore();
    check('T1 — no inquirer prompt fired', inq1.calls.length === 0,
      `inquirer was called ${inq1.calls.length} times`);
    ...
    setStdinIsTTY(true);
    const inq2 = makeInquirerMock(true);
    const capt2 = captureConsole();
    const ops2 = createRuntimeOps(makeDeps({ inquirerInstance: inq2.instance }));
    try { await ops2.translateMod(tmpModDir, {}); } catch (e) {}
    capt2.restore();
    check('T2 — no inquirer prompt fired', inq2.calls.length === 0);
    ...
    setStdinIsTTY(false);
    const inq3 = makeInquirerMock(true);
    const capt3 = captureConsole();
    const ops3 = createRuntimeOps(makeDeps({ inquirerInstance: inq3.instance }));
    try { await ops3.translateMod(tmpModDir, {}); } catch (e) {}
    capt3.restore();
    check('T3 — no inquirer prompt fired', inq3.calls.length === 0);
    ...
    process.argv = ORIGINAL_ARGV.slice();
    setStdinIsTTY(true);
    const inq4 = makeInquirerMock(true);
    const capt4 = captureConsole();
    const ops4 = createRuntimeOps(makeDeps({ inquirerInstance: inq4.instance }));
    try { await ops4.translateMod(tmpModDir, {}); } catch (e) {}
    capt4.restore();
    check('T4 — no inquirer prompt fired', inq4.calls.length === 0);
    ...
    process.argv = ORIGINAL_ARGV.slice();
    setStdinIsTTY(true);
    const inq5 = makeInquirerMock(false);
    const capt5 = captureConsole();
    const ops5 = createRuntimeOps(makeDeps({ inquirerInstance: inq5.instance }));
    const t5Result = await ops5.translateMod(tmpModDir, {});
    capt5.restore();
    check('T5 — inquirer.prompt fired exactly once', inq5.calls.length === 1);
    ...
    const inq6b = makeInquirerMock(false);
    const capt6b = captureConsole();
    const ops6b = createRuntimeOps(makeDeps({
      inquirerInstance: inq6b.instance,
      get: () => sessionConfirmed,
      set: (v) => { sessionConfirmed = !!v; }
    }));
    try { await ops6b.translateMod(tmpModDir, {}); } catch (e) {}
    capt6b.restore();
    check('T6.1 — second translateMod in same session does NOT prompt', inq6b.calls.length === 0);
    ...
    const inq7 = makeInquirerMock(false); // answer MUST NOT be reached
    const capt7 = captureConsole();
    const ops7 = createRuntimeOps(makeDeps({ inquirerInstance: inq7.instance }));
    try { await ops7.translateMod(tmpModDir, {}); } catch (e) {}
    capt7.restore();
    check('T7 — inquirer NOT prompted (persisted flag honored)', inq7.calls.length === 0);
    ...
    process.argv = ORIGINAL_ARGV.slice();
    setStdinIsTTY(true);
    const inq9 = makeInquirerMock(true);
    const capt9 = captureConsole();
    const ops9 = createRuntimeOps(makeDeps({ inquirerInstance: inq9.instance }));
    try { await ops9.translateMod(tmpModDir, { dryRun: true }); } catch (e) {}
    capt9.restore();
    check('T9 — no inquirer prompt during dry-run', inq9.calls.length === 0);
    ...
    process.argv = ORIGINAL_ARGV.slice();
    setStdinIsTTY(true);
    const inq10 = makeInquirerMock(true);
    const capt10 = captureConsole();
    const ops10 = createRuntimeOps(makeDeps({ inquirerInstance: inq10.instance }));
    ...
    check('T10 — no inquirer prompt fired', inq10.calls.length === 0);
