...
function createProviderClients(ctx) {
  ...
  function getBatchProfile(provider, model, mode = 'translate') {
    ...
    // FCM: local daemon proxy — moderate batches (routes to underlying free models)
    if (provider === 'fcm')         return { maxItems: mode === 'polish' ? 12 : 20, maxChars: 2600 };

    // NVIDIA NIM: Nemotron models have good context window but moderate speed
    if (provider === 'nvidia' && isLarge) return { maxItems: mode === 'polish' ? 16 : 28, maxChars: 3200 };
    if (provider === 'nvidia')            return { maxItems: mode === 'polish' ? 14 : 22, maxChars: 2800 };
