...
### Risk Routing (erweitert v0.19.7-19.06)
...
## Polish Arbiter (NEU v0.19.7-19.06)
...
## CLI Progress (NEU v0.19.7-19.06)
...
## Architektur (Stand v0.19.7-19.06)
...
## 📡 FCM — free-coding-models Integration (Level 2)

FCM liefert Echtzeit-Model-Rankings (Tier, Ping, Stability) und stellt
einen OpenAI-kompatiblen Router-Daemon auf `localhost:19280/v1` bereit.

**Live-Stats Pipeline:**
1. `fetchFcmModelRankings()` → pulled FCM-JSON (`free-coding-models --json --best`)
2. `enhanceModelListWithFcm()` → merged Rankings in SyxBridge's Model-Selektion
3. Provider `"fcm"` → als Fallback/Option in router.js + dispatcher.js

**Ohne FCM:** SyxBridge läuft normal mit statischen Fallback-Listen.
