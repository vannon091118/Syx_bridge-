...
async function shouldSkipFile(filePath, outPath) {
...
async function writeTranslatedFile(job, modOutputPath, translations) {
  ...
  if (await shouldSkipFile(job.filePath, outPath)) return { skipped: true };
...
async function main() {
  ...
  const isSingleAuto = process.argv.includes('--single-auto');
  ...
  runtimeOps = createRuntimeOps({
    ...
    getMajorVersion: async (dir) => {
      try {
        ...
        return versions.length > 0 ? versions[0] : 70;
      } catch (e) { return 70; }
    },
  ...
  translationRuntime = createTranslationRuntime({
    axios, config: CONFIG, routingEngine, logPayload, withRetry, sleep,
    getApiKey: (p) => configRuntime.getApiKey(p),
    rotateApiKey: (p) => configRuntime.rotateApiKey(p),
    extractErrorMessage, parseBatchResponse, buildBatchPrompt, buildProofreadPrompt, protectPlaceholders, restorePlaceholders, isProperNoun, classifyPath, restoreAndValidateTranslation, shouldTranslate, stripJsonFence, getGrammarContext, getModelForProvider, getGeminiModelName, dbGet, dbAll, dbRun,
  ...
  if (isGui) {
    ...
    global.guiServer.on('idle', async () => {
      console.log('[GUI] Sitzung beendet.');
      await stopOllama();
  ...
  while (true) {
    ...
    // else if (menu.action === 'single') await startSingleManual(planner, options);
