...
function createTranslationRuntime(options) {
  ...
  function buildGeminiRequest(prompt, mode = 'text', expectedCount = 0) {
    ...
    if (expectedCount > 0) {
      ...
      request.responseSchema = buildGeminiSchema(expectedCount, mode);
    }
...
`.trim();
  ...
  async function executeStageRequest(stage, route, prompt, options = {}, attemptCount = 0) {
    ...
    try {
      if (provider === 'gemini') {
        if (!key) throw new Error('Gemini API Key fehlt.');
        const url = `https://generativelanguage.googleapis.com/v1beta/models/${getGeminiModelName(model)}:generateContent?key=${key}`;
      ...
      if (provider === 'ollama') {
        ...
        const response = await axios.post(`${config.OLLAMA_URL}/api/chat`, {
          ...
          stream: false,
          format: 'json'
        }, { headers, timeout: mode === 'flags' ? 30000 : 60000 });
        const raw = response.data.message.content;
        logPayload(provider, `RESPONSE [${stage}]`, raw);
        return parseRaw(raw);
      }
  ...
  async function getCachedTranslations(items) {
    ...
    for (const entry of entries) {
      const sourceHash = getEntryHash(entry);
      const row = sourceHash
        ? await dbGet('SELECT translation, audit_stage, flagged, flag_reason, provider, quality_score, source_hash FROM translations WHERE target_lang = ? AND (source_text = ? OR source_hash = ?) ORDER BY CASE WHEN source_text = ? THEN 0 ELSE 1 END LIMIT 1', [config.TARGET_LANG, entry.source, sourceHash, entry.source])
        : await dbGet('SELECT translation, audit_stage, flagged, flag_reason, provider, quality_score, source_hash FROM translations WHERE source_text = ? AND target_lang = ?', [entry.source, config.TARGET_LANG]);
      if (row) {
        cached.set(entry.source, {
  ...
  async function fixGrammarBatch(items, stage = 'audit', attemptCount = 0) {
    ...
    try {
      ...
      return fixGrammarBatch(items, stage);
    }
  }
  ...
  async function ensureTranslations(texts, options = {}) {
    ...
    uniqueTexts.forEach(t => {
      if (cachedData.has(t)) {
        ...
        const needsRefresh = data.flagged && data.translation === t && !isLikelyGermanText(t);
        const hashMismatch = data.sourceHash && sourceEntry.sourceHash && data.sourceHash !== sourceEntry.sourceHash;
        if (!needsRefresh && !hashMismatch) {
